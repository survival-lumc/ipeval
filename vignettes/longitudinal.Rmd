...
This vignette demonstrates how to evaluate predictions under longitudinal treatment strategies in the presence of time-dependent confounding.

For more detail, see [Keogh and Van Geloven (2024)](https://doi.org/10.1097/EDE.0000000000001713), from which the methods in this package are implemented.

We assume that an evaluation dataset (df_val) is available, as well as risk estimates corresponding to one or more treatment strategies. Throughout this vignette, we use data generated from the causal structure shown below.
...
In [the previous vignette](longitudinal-data-and-cox-models.html), we show how such predictions can be obtained from a marginal structural Cox model (and how longitudinal data could be simulated). Here, we focus on evaluating those predictions. We will start from a wide-form dataset called df_val.
...
simulate_longitudinal_data <- function(n, n_visits = 5, visit_times = 0:4, seed) {
  ...
  for (i in 1:n_visits) {
    ...
    # To simulate outcome data (survival time + status) with these time varying variables, we simulate a
...
We will use the `ip_score_long()` function, which evaluates risk estimates under longitudinal interventions using inverse-probability weighting. It requires:

* an outcome dataset with one row per subject, with survival time and status,
* a dataset in long format, containing data on observed treatment and covariates, with one row per subject and visit,
* a model describing the treatment assignment mechanism.

The long dataset must include all variables that are confounders of the treatment-outcome relation. In our example, we assume the treatment model needs to include the value of L at the current visit and the treatment value at the previous visit, so the variables L, A, and A_lag_1 are required.

This package provides the convenience functions `wide_to_long()` and `add_lag_terms()` to help reshape your data in this way.
...
We can use the function `ip_score_long()` to evaluate the predictions under the never-treat strategy as follows:
...
The treatment strategy does not need to be "always" or "never" treat. For example, the following strategy specifies treatment at the first two visits and leaves subsequent treatment unconstrained:
...
In the previous examples, we did not specify the censoring mechanism. By default, censoring is assumed to be (marginally) independent of the time-to-event and the censoring weights are estimating using Kaplan-Meier. It is also possible to specify a Cox censoring model where censoring may depend on (time-varying) covariates. These variables should then be available as columns in ```data_long```. As a demonstration, this can be achieved as follows:
...
Note that the coefficients of the censoring model are estimated close to 0. This is expected as we simulated the data assuming a (marginally) independent censoring process.
