test_that("test if assumption output and graphics is correct", {
  ...
  expect_equal(output, "Estimation of the performance of the prediction model in a pseudopopulation where everyone's treatment trt was set to 1.The pseudopopulation is constructed from 5472 (54.7%) subjects ($pseudopop) in data who indeed received treatment level 1. These subjects are reweighted to represent the full target population under a hypothetical intervention in which everyone received this treatment level.The following assumptions must be satisfied for correct inference:Causal assumptions:- Conditional exchangeability: after adjustment for the covariates used to construct the inverse probability of treatment weights (IPTW), i.e., {conf1, conf2}, there is no unmeasured confounding for the relation between treatment and outcome.- Conditional positivity: the probability of receiving treatment level 1 should be greater than zero for each value (combination) of the variable(s) {conf1, conf2} that is observed in the full population. The distribution of IPT-weights can be assessed with $ipt$weights[$pseudopop$ids].- Consistency: the observed outcome under the received treatment level equals the potential outcome under that treatment level. This includes the assumption of no interference between subjects.Modeling assumptions:- Correctly specified propensity model. Estimated treatment model islogit(trt) = 0.21 + 0.47*conf1 + 0.26*conf2. See also $ipt$model.Performance estimates:      model brier null model 0.237       pred 0.217")
