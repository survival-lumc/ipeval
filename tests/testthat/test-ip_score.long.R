...
test_that("bootstrap long", {
  set.seed(123)
  n_dev <- 1000
  n_val <- 10000


  df_dev <- generate_long_data_cox(n_dev, seed = 10)
  df_dev_long <- make_dev_long(df_dev)
  iptw <- ipt_weights(df_dev_long, A ~ L * A_lag_1)$weights
  ...
  df_val <- generate_long_data_cox(n_val, seed = 11)
  ...
  for (m in metrics) {
    lower <- sapply(score0$bootstrap$results[[m]], function(x) x[[1]])
    upper <- sapply(score0$bootstrap$results[[m]], function(x) x[[2]])
    expect_true(all(lower <= score0$score[[!!m]]))
    expect_true(all(upper >= score0$score[[!!m]]))
    expect_false(all(upper == lower))
  }
})
